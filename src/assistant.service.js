...
module.exports = class Assistant {
    ...
    getUserLocation(agent) {
        ...
        if (requestedPermission !== 'DEVICE_PRECISE_LOCATION') {
            return agent.add(
                "J'ai besoin de votre autorisation pour vous localiser."
            );
        }
        ...
        agent.add(conv);
        return agent.add(
            `Vous êtes à ${coordinates.latitude}, ${coordinates.longitude}.`
        );
    }
    ...
    saveStation(agent) {
        ...
        if (conv.user.verification !== 'VERIFIED') {
            return agent.add(
                'Désolé, je ne peux pas sauvegarder votre arrêt préféré.'
            );
        }
    ...
    async getWaitingTime(agent) {
        ...
        if (
            agent.conv().user.storage.arret &&
            agent.conv().user.storage.arret.length
        )
        ...
        const nameStation = await tan.getSimilarStationsName(
            arret,
            tan.parseStationsToList(tramStations),
            1
        );
        const details = await tan.getWaitingTimeFromStation(
            nameStation[0].name,
            'name'
        );

        // Sélectionne seulement les tramways
        const onlyTram = details.filter(
            station => station.ligne.typeLigne == 1
        );
        ...
        for (
            let i = 0;
            i < onlyTram.length && (sens1 == 0 || sens2 == 0);
            i++
        ) {
    ...
    async getCloseStations(agent) {
        ...
        for (var i = 0; i < stations.length; i++) {
            const isTram = stations[i].ligne.filter(
                ligne => parseInt(ligne.numLigne) <= 3
            );
            if (isTram.length > 0) {
                // Si c'est une station où passent les tramways
                if (i != 0) resultAgent += ', ';
                resultAgent +=
                    stations[i].libelle + ' à ' + stations[i].distance;
                nbStops++;
            }
        }
    ...
    async getDetailsStations(agent) {
        ...
        const times = await tan.getTimesFromStation(
            nameStation[0].name,
            'name',
            ligne,
            direction
        ...
        agent.add(
            'Voici le prochain horaire de passage: ' +
                times.prochainsHoraires[0].heure +
                times.prochainsHoraires[0].passages[0]
        );
    ...
    async getDetailsNextStation(agent) {
        ...
        if (times[nbTimesAsked] && times[nbTimesAsked].passages[nbTimesAsked])
            agent.add(
                'Voici le prochain horaire de passage: ' +
                    times[nbTimesAsked].heure +
                    times[nbTimesAsked].passages[nbTimesAsked]
            );
        else {
            nbTimesAsked = 0;
